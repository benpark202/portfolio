const theme = () => {
  ...
  const themeSun = themeToggle?.querySelector(".theme-sun") ?? null;
  const themeMoon = themeToggle?.querySelector(".theme-moon") ?? null;
  ...
  const clearActiveTransition = () => {
    if (activeTransition) {
      ...
      activeTransition.circle.remove();
  ...
  if (themeSun) {
    themeSun.addEventListener("animationend", (event) => {
      if (event.animationName === "sun-double-spin") {
        themeSun.classList.remove("is-spinning");
      }
    });
  }

  if (themeMoon) {
    themeMoon.addEventListener("animationend", (event) => {
      if (event.animationName === "moon-spin-into-sun") {
        themeMoon.classList.remove("is-spinning");
      }
    });
  }

  const spinSymbol = (symbol) => {
    if (!symbol || reduceMotionQuery.matches) return;
    symbol.classList.remove("is-spinning");
    void symbol.offsetWidth;
    symbol.classList.add("is-spinning");
  };

  if (!themeToggle) return;
  themeToggle.addEventListener("click", () => {
    const visibleTheme = root.getAttribute("data-theme") === "dark" ? "dark" : "light";
    const logicalTheme = activeTransition?.nextTheme ?? visibleTheme;
    const nextTheme = logicalTheme === "dark" ? "light" : "dark";
    const isReverseToLight = nextTheme === "light";
    themeSun?.classList.remove("is-spinning");
    themeMoon?.classList.remove("is-spinning");
    if (nextTheme === "light") {
      spinSymbol(themeMoon);
    } else {
      spinSymbol(themeSun);
    }
    ...
    const fallbackShapeIndices = transitionShapes
      .map((_shape, index) => index);
    const shapePool = availableShapeIndices.length > 0 ? availableShapeIndices : fallbackShapeIndices;
    const randomShapeIndex = shapePool[Math.floor(Math.random() * shapePool.length)];
    ...
    circle.style.backgroundColor = isReverseToLight
      ? themeSurfaceColors[visibleTheme] ?? themeSurfaceColors.dark
    ...
    body.style.setProperty("transition", "background-color 0ms linear, color 220ms linear");
    body.style.setProperty(
      "--text-color",
      themeTextColors[nextTheme] ?? themeTextColors.light
    );
    ...
    const finalizeTransition = () => {
      if (activeTransition?.circle === circle) {
        activeTransition = null;
      }
      circle.remove();
      body.classList.remove("theme-switching");
    ...
    const stepTransition = (nowMs) => {
      ...
      if (activeTransition?.circle !== circle) return;
      activeTransition.frameId = window.requestAnimationFrame(stepTransition);
    };
