...
const WORD_BANK: HangmanWord[] = [
  { word: "TYPESCRIPT", hint: "The language used for this project." },
  { word: "ACCESSIBLE", hint: "A good interface works for everyone." },
  { word: "RESPONSIVE", hint: "Design that adapts to screen size." },
  { word: "COMPONENT", hint: "A reusable block of UI." },
  { word: "BLUEPRINT", hint: "A plan before building." },
];
...
const hangmanParts = Array.from(document.querySelectorAll<HTMLElement>("[data-part]"));
...
const getActiveWord = (): HangmanWord => WORD_BANK[state.activeWordIndex];

const normalizeLetter = (value: string): string => value.trim().toUpperCase().slice(0, 1);

const renderWord = (): void => {
...
const updateFinishedState = (): void => {
...
const handleGuess = (rawLetter: string): void => {
  ...
  if (activeWord.includes(letter)) {
    ...
    const guessedWord = activeWord
      .replace(/\s+/g, "")
      .split("")
      .every((item) => state.guessedLetters.has(item));
